...
/// app en het pad staat in het verslag (`outputPath`); het komt in de fs-scope
/// zodat de webview het kan tonen. Voortgang komt als event
/// `cad-preview-progress`, zodat een lopend voorbeeld de balk van een echte
/// import niet verstoort; afbreken gaat met `cancel_cad_import`.
#[tauri::command]
pub async fn preview_cad_import(
    ...
    state.done(&job_id);
    let result = result??;
    let _ = app.fs_scope().allow_file(&result.output_path);
    Ok(result)
}
