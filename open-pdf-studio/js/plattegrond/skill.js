...
export const SPOUWMUUR_INSTRUCTIE =
  '   Wandjoin: wanden die elkaar raken sluit de app zelf af. Samenvallende eindpunten gaan in verstek (elk materiaal). Einden die een wand van HETZELFDE materiaal net passeren of net niet halen, worden tot een dichte hoek getrimd; met een ander materiaal gebeurt dat niet, zodat een laag nooit door een andere laag steekt. Een wand die op een doorlopende wand stopt (eindpunt op de hartlijn, op het vlak, er net voor of erin) wordt een T: bij hetzelfde materiaal vloeien de wanden in elkaar over, bij een ander materiaal stopt hij stomp tegen het vlak met een naad. Zet de join aan een uiteinde alleen uit als dat einde precies moet blijven zoals getekend, met een eigen kap en zonder verstek, T of afkappen: joinStart:false en/of joinEnd:false (begin/eind van de loop), of achteraf app_update_annotation {id, props:{noJoinStart:true}} of {noJoinEnd:true} (true = join uit, false = weer aan).\n' +
  ...
  '   - Hoeken per laag: laat in een hoek de hartlijnen van DEZELFDE laag van beide gevels in één punt samenvallen (metselwerk op metselwerk, isolatie op isolatie, kalkzandsteen op kalkzandsteen). Dan verstekt elke laag met zichzelf en steekt geen laag door een andere laag of door de spouw. Moet een laag stomp tegen een doorgaande laag stoppen, laat hem dan op dat vlak eindigen: bij een ander materiaal wordt dat vanzelf een stompe aansluiting met naad, bij hetzelfde materiaal vloeien ze in elkaar over (wil je daar toch een losse kap, zet dan de join uit).\n' +
  ...
  '   - Binnenwanden sluiten aan op het binnenblad: laat ze eindigen op het binnenvlak (of de hartlijn) van de kalkzandsteen, niet op de hartlijn van de hele gevel. De app maakt er een T van: kalkzandsteen op kalkzandsteen vloeit in elkaar over, een binnenwand van een ander materiaal stopt stomp tegen het binnenvlak. Zet daar de join NIET uit, anders krijgt het einde weer een losse kap.\n' +
