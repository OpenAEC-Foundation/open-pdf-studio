...
import {
  dekkingUitProcent, onderleggerFactor, onderleggerVak, paginaNoemer, rasterPlan, witNaarDoorzichtig,
...
/** Pad voor de PDF die de import maakt: in de tijdelijke map van het systeem. */
export async function tijdelijkPdfPad(bron) {
  const naam = pdfNaamVoor(bron);
  if (isTauri() && window.__TAURI__?.path?.tempDir) {
    const map = await window.__TAURI__.path.tempDir();
    const sep = map.endsWith('\\') || map.endsWith('/') ? '' : '/';
    const pad = `${map}${sep}opds-import-${Date.now()}-${naam}`;
 ...
 * binnen het gedeelde geheugen van de renderer.
 * @returns {Promise<{img: HTMLImageElement, dataUrl: string, plan: object}>}
 */
async function rasterBlad(pad, bladPt, dpi) {
  const plan = rasterPlan(bladPt, dpi);
  ...
  for (const strook of plan.stroken) {
    const regio = {
      x: 0,
      y: strook.yPx / plan.schaal,
      width: bladPt.breedte,
      height: strook.hoogtePx / plan.schaal,
    };
    const beeld = await rasterBytesVanPad(pad, regio, plan.schaal);
 ...
 * @param {number} [o.dpi]
 * @returns {Promise<{annotatie: object, pagina: number, opSchaal: boolean, begrensd: boolean}>}
 */
export async function legTekeningOpPagina(pad, o = {}) {
  ...
  try {
    ...
    if (o.alsAfbeelding === true) {
      const { img, dataUrl, plan } = await rasterBlad(pad, bladPt, o.dpi);
    ...
    return { annotatie, pagina: paginaNr, opSchaal: vak.opSchaal, begrensd };
  } finally {
    await ruimOp(pad);
  }
}
...
export async function ruimOudeImportsOp(ouderDanMs = 24 * 60 * 60 * 1000) {
  ...
  try {
    ...
    let opgeruimd = 0;
    for (const item of await fs.readDir(map)) {
