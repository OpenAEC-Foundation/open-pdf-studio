 ...
 */
export function lagenKeuze(lagen, uit, inst) {
  const namen = (lagen || []).filter((l) => uit?.has?.(l.name.toUpperCase())).map((l) => l.name);
  if (inst?.includeOffLayers && inst?.layersAsOcg !== false) return { excludedLayers: [], hiddenLayers: namen };
...
export function importArgumenten(inst, o) {
  const args = {
    ...
    measure: inst.measure !== false,
    modelMatrix: inst.modelMatrix !== false,
    colors: inst.colors,
