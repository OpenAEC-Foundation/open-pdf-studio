...
import {
  ...
  eenheidMelding, effectiefDoel, filterLagen, gebiedMaatMm, gebiedUitLagen, getalTekst, grootteTekst, importArgumenten, lagenKeuze,
  ...
  schoonZoekpaden, sorteerLagen, standaardUitgesloten, vensterGeldig, zetNietPlotbaar, MAX_PAGINA_MM, papiermaatMm,
} from '../../../pdf/cad-import-logica.js';
import {
  annuleerImport, controleerZoekpaden, importeerTekening, importGrenzen, kiesTekening, kiesZoekpad, laatTekeningLos,
  legTekeningOpPagina, luisterImportVoortgang, meetschaalHuidigePagina, nieuwJobId, openImportAlsNieuwDocument, ruimOp,
  ruimOudeImportsOp, tijdelijkPdfPad, verkenTekening, voegImportToeAanDocument, volgendeTekening, zoekBuitenBestanden,
...
export default function CadImportDialog(props) {
  ...
  const lagenGebied = createMemo(() => gebiedUitLagen(lagen(), uit()) || ruimte()?.bounds || null);
  ...
  const gebied = createMemo(() => {
    ...
    if (inst.area === 'window') return vensterGeldig(venster) ? [venster.x0, venster.y0, venster.x1, venster.y1] : null;
    if (inst.area === 'limits' && limitsBruikbaar()) return r.limits;
    return lagenGebied();
  });
  ...
  const omzetArgumenten = (outputPath) => {
    ...
    return importArgumenten({ ...inst, ...papierInstellingen(inst, isModel(), layoutKeuze) }, {
      path: pad(),
      outputPath,
  ...
  async function importeer() {
    ...
    try {
      ...
      if (!extra.length) {
        if (opPagina) updateStatusMessage(t('cadImport.underlayDone', { page: getal(opPagina.pagina) }));
        closeDialog('cad-import');
        return;
      }
    ...
    >
      ...
      <fieldset class="cad-fieldset cad-panels" disabled={bezig() || !scan()}>
        ...
        <div id="cad-panel-layers" role="tabpanel" aria-labelledby="cad-tab-layers" aria-hidden={tab() !== 'layers'} class="cad-tab-content" classList={{ active: tab() === 'layers' }}>
            ...
            <div class="cad-layer-head">
              ...
              <For each={KOLOMMEN}>
                {(k) => (
                  <button
                    type="button"
                    class={`cad-sort ${k.cls}`}
                    title={t('cadImport.sortBy')}
            ...
            <For each={zichtbareLagen()}>
              {(laag) => (
                <label class="cad-layer-row" role="listitem">
                  ...
                  <span class="cad-col-color"><span class="cad-swatch" style={{ background: laag.color || '#ffffff' }} /></span>
                  <span class="cad-col-name" title={laag.name}>{laag.name}</span>
                  <span class="cad-col-count">{getal(laag.objects || 0)}</span>
                  <span class="cad-col-state">{statusTekst(laag)}</span>
                </label>
              )}
            </For>
          </div>
          ...
          <label class="cad-check" classList={{ 'cad-disabled': !inst.layersAsOcg }}>
            <input type="checkbox" checked={inst.includeOffLayers && inst.layersAsOcg} disabled={!inst.layersAsOcg}
        ...
        <div id="cad-panel-coordinates" role="tabpanel" aria-labelledby="cad-tab-coordinates" aria-hidden={tab() !== 'coordinates'} class="cad-tab-content" classList={{ active: tab() === 'coordinates' }}>
          ...
          <label class="cad-check"><input type="checkbox" checked={inst.measure} onChange={(e) => setInst('measure', e.target.checked)} /> {t('cadImport.measure')}</label>
          <label class="cad-check"><input type="checkbox" checked={inst.modelMatrix} onChange={(e) => setInst('modelMatrix', e.target.checked)} /> {t('cadImport.modelMatrix')}</label>
        </div>
