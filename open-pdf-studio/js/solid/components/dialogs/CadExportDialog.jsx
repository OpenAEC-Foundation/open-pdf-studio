  ...
  async function exporteer() {
    ...
    try {
      ...
      if (afgebroken) {
        ...
        const getal = totaal.objecten.toLocaleString();
        const grootte = grootteTekst(totaal.bytes);
        setMelding({
          soort: 'ok',
          tekst: totaal.bestanden === 1
            ? t('cadExport.doneOne', { path: laatstePad(), objects: getal, size: grootte })
            : t('cadExport.doneMany', { files: totaal.bestanden, objects: getal, size: grootte }),
        });
      }
    } catch (e) {
    ...
    >
      ...
      <fieldset class="cad-fieldset" disabled={bezig()}>
        ...
        <div class="cad-tab-content" classList={{ active: tab() === 'layers' }}>
            ...
            <For each={zichtbareLagen()}>
              {(laag) => (
                <label class="cad-layer-row" role="listitem">
                  ...
                  <span class="cad-col-count">{laag.entities.toLocaleString()}</span>
          ...
          <div class="cad-note">
            <Show when={scan().bezig} fallback={
              <Show when={scan().fout} fallback={t('cadExport.scanned', { objects: totaalObjecten().toLocaleString(), layers: lagen().length })}>
          ...
          <Show when={!scan().bezig && totaalObjecten() > MAX_ENTITEITEN}>
            <div class="cad-note cad-note-warn">{t('cadExport.tooLargeWarn', { objects: totaalObjecten().toLocaleString(), limit: MAX_ENTITEITEN.toLocaleString() })}</div>
          </Show>
        </div>
        ...
        <div class="cad-tab-content" classList={{ active: tab() === 'area' }}>
          ...
          <Show when={gebiedKeuze() === 'window'}>
            ...
            <div class="cad-row">
              <label class="cad-label">X / Y (mm)</label>
      ...
      <Show when={teGroot()}>
        <div class="cad-question" role="alertdialog">
          ...
          <div>{t('cadExport.tooLargeText', { page: teGroot().pagina, objects: teGroot().entities.toLocaleString(), limit: teGroot().limit.toLocaleString() })}</div>
