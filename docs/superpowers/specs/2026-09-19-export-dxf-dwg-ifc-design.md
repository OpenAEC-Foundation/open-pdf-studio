...
Ontwerp, 19 september 2026. Issue #400. Status (21 september 2026): **in
uitvoering** — de export (fase 1 en het exportvenster) en de import van DWG en
DXF met importvenster (fase 5, zie Deel J) zijn gebouwd en gemeten; IFC is
alleen ontworpen.
...
| Viewports met papiermaat, modelhoogte (→ schaal), draaiing, bevroren lagen | 82 bestanden | Ja: schaal per viewport. Let op: in DWG is het viewportnummer altijd 0, dus de "hele-papier"-viewport is alleen aan zijn maat te herkennen |
...
**In Rust, met `lopdf`** (0.34, al in de app en in `open-pdf-render`; er is een
precedent in `print_windows.rs:530`), als module `import` in `open-pdf-cad`.
...
- **Tekst.** Fase 1: de standaardletter Helvetica (geen inbedding nodig,
  ...
  tekst blijft doorzoekbaar en selecteerbaar. Fase 2: een vervangingstabel
  (SHX-letters → een ingebedde open letter) en tekens buiten Latin-1.
- **Arceringen.** Effen → gevuld pad met gaten (oneven-pariteit).
  Patroonarcering → patroonlijnen genereren en op de grens knippen (fase 2);
  in fase 1 alleen de omtrek, of overslaan.
- **Blokken als formulier-XObject** (fase 2): een veelgebruikt blok één keer
  in de PDF en per INSERT een `cm … Do`. Scheelt fors in bestandsgrootte; in
  fase 1 gewoon uitvouwen.
...
| *Voorstel:* lagenselectie onthouden per bestandsnaam | Zelfde tekening opnieuw importeren na een revisie | uit | O | Selectie op naam teruggezet; nieuwe lagen volgen de standaardregel |
...
| Meetschaal schrijven | `/VP` + `/Measure` per (viewport van de) pagina | aan | V | De maatvoering van de app klopt direct (E4) |
| *Voorstel:* modelmatrix bewaren | Afbeelding pagina → model in de viewport | aan | V | Coördinaten uitlezen en terugschrijven naar CAD in de oorspronkelijke coördinaten |

#### Tabblad Weergave — *voorstel*

| Instelling | Keuzes | Standaard | Herkomst | Doorwerking |
|------------|--------|-----------|----------|-------------|
| Kleuren | Volgens bestand · alles zwart · grijstinten | Volgens bestand | O | Index 7 en wit worden zwart op papier; anders de kleur van entiteit of laag |
| Lijndiktes | Volgens bestand · kleurentabel · vaste dikte | Volgens bestand | O | Entiteit → laag → standaard 0,25 mm. Kleurentabel: eigen tabel kleur → dikte, op te slaan als voorinstelling (vervangt de pentabel die niet in het bestand zit) |
| Factor en minimum | Vermenigvuldiger en ondergrens | 1,0 en 0,05 mm | O | `w = max(min, dikte × factor)` |
| Lijntypen | Volgens bestand · alles doorgetrokken | Volgens bestand | O | Patroon × `$LTSCALE` × schaal van de entiteit → `d`; bij papierruimte volgens `$PSLTSCALE` |
| Tekst | Als tekst · als omtrek · weglaten | Als tekst | O | Tekst blijft doorzoekbaar; omtrek geeft het exacte beeld van een SHX-letter |
| Lettertypevervanging | Tabel "in de tekening" → "in de PDF" | SHX → schreefloos, TrueType → zelfde naam als geïnstalleerd, anders schreefloos | B (lijst) / O (keuzes) | Het venster toont welke letters ontbreken |
| Arceringen | Alles · alleen effen · alleen omtrek · weglaten | Alles (patronen in fase 1 als omtrek) | O | Zie E3 |
| Maatvoering | Meenemen · weglaten | Meenemen | O | Het anonieme blok van de DIMENSION invoegen |
| Blokken en attributen | Uitvouwen (vast) · attributen tonen ja/nee | ja | O | Onzichtbare attributen blijven weg |
| Externe verwijzingen | Laden indien gevonden · overslaan | Laden; zoekpad = map van het bestand | O | Niet gevonden → melding met de bestandsnaam, de import gaat door |
| Afbeeldingen | Insluiten · overslaan | Insluiten indien gevonden | O | Als beeld-XObject, geknipt op de kaderlijn |
| Punten (POINT) | Weglaten · als stip | Weglaten | O | — |

#### Onderin — *voorstel*

| Onderdeel | Beschrijving |
|-----------|--------------|
| **Doel** | Nieuw document · nieuwe pagina in het huidige document (na de huidige) · onderlegger op de huidige pagina, met dekking (standaard 50%) en "onder de bestaande inhoud". Beginwaarde: onthouden |
...
|---------|-----------|-----------|--------|
| Lagen | Lijst van PDF-lagen (of de afgeleide kleur/dikte-lagen) met aan/uit, aantal objecten, kleur; alles/niets, zoekfilter | alles aan | lijst komt uit een voorscan van de pagina (`extract_page` met een tellende afnemer, per pagina gecachet) — fase 2 |
| Lagen | Laagindeling: PDF-lagen · kleur + lijndikte · één laag | PDF-lagen indien aanwezig | gebouwd |
| Coördinaten | Pagina's: huidige · alle · bereik (`parsePageRange` bestaat al); één bestand per pagina, of naast elkaar in één bestand | huidige | fase 2 |
| Coördinaten | Gebied: hele pagina · schaalgebied · venster | hele pagina | fase 2 |
| Coördinaten | Eenheden en schaal: papier-mm (1:1) · ware grootte volgens de meetschaal · vrije schaal; tekeneenheid mm, cm of m | ware grootte als de pagina een schaal heeft, anders papier | schaal gebouwd; eenheidskeuze fase 2 |
| Coördinaten | Oorsprong: linksonder van de pagina · linksonder van het gebied · oorspronkelijke modelcoördinaten (na een import) · zelf opgeven | linksonder van de pagina | fase 2 |
...
| Geometrie | Annotaties van de app: mee op eigen lagen · weglaten | mee | fase 2 |
| Geometrie | Afbeeldingen: als verwijzing met PNG naast het bestand · weglaten | weglaten | fase 2 |
| Onderin | Formaat (DXF tekst · DXF binair · DWG · IFC) en versie; voorinstellingen; samenvatting met aantallen en een waarschuwing boven de omvanggrens; Exporteren · Annuleren; voortgang met Afbreken | DXF tekst, 2013 | formaat en versie gebouwd |
...
verantwoord. `lopdf` zit er al in. Als de import in de sidecar komt in plaats
...
| Import: viewports, externe verwijzingen, SHX-letters, patroonarceringen, pentabellen | middel | Gefaseerd (deel J); het venster meldt wat is overgeslagen |
...
|---|---|---|
| Bezoeken (`maxVisits`) | 40 miljoen | één gedeelde teller voor alle wandelingen van een verkenning (modelruimte en elk blad), en voor alle wandelingen van een omzetting (grenzen bepalen én tekenen, voor elke gekozen ruimte) |
...
Nog open, voor fase 6: de viewports van een layout kwamen in een van de
geteste DWG's niet op de pagina (de verkenning ziet 7 viewports, de pagina
toont alleen het papier); voorbeeldweergave, onderlegger, kleurentabel,
lettertypevervanging, externe verwijzingen, afbeeldingen, blokken als
formulier-XObject, en de MCP-opdracht `app_import_cad`.
