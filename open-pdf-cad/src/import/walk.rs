...
use super::curves::{add_elliptic_arc, bulge_arc, catmull_rom, ccw_sweep, sample_bspline, PagePath, Xform3};
use super::hatch;
...
/// MINSERT, opgeteld over alle wandelingen (grenzen bepalen en tekenen, voor
/// elke gekozen ruimte). Een gewone zware tekening blijft daar ruim onder;
/// een blok dat zichzelf tweemaal invoegt zou zonder grens 2^24 keer
/// uitvouwen.
...
pub struct WalkStats {
    ...
    /// entiteit, elke uitgevouwen blokverwijzing en elke cel van een MINSERT.
    pub visits: u64,
    ...
    /// toegestane mappen, of boven het aantal bestanden van één import.
...
impl<'a> Walker<'a> {
    ...
    fn entity(&mut self, entity: &'a EntityType, ctx: &Ctx<'a>, sink: &mut dyn Sink) {
        ...
        match entity {
            ...
            EntityType::Arc(arc) => {
                ...
                let (t0, t1) = ccw_sweep(arc.start_angle, arc.end_angle);
            ...
            EntityType::Ellipse(ellipse) => {
                ...
                let (t0, t1) = if (ellipse.end_parameter - ellipse.start_parameter).abs() >= std::f64::consts::TAU - 1e-9 {
                    ...
                    ccw_sweep(ellipse.start_parameter, ellipse.end_parameter)
                };
                add_elliptic_arc(&mut path, c, u, v, t0, t1, true);
                self.emit_stroke(layer.name, &stroke, &path, sink);
                self.path = path;
    ...
    fn hatch(&mut self, entity: &acadrust::entities::Hatch, layer: &LayerInfo<'a>, ctx: &Ctx<'a>, sink: &mut dyn Sink) {
        ...
        if boundary.is_empty() || !boundary.is_finite() {
            return;
        }
    ...
    fn draw_line_of_text(
        ...
        let tm = glyph
            ...
            .then(&m);
        let bounds = [
            tm.apply(Point::new(0.0, -0.25)),
            tm.apply(Point::new(width_em, -0.25)),
            tm.apply(Point::new(width_em, 1.0)),
            tm.apply(Point::new(0.0, 1.0)),
        ];
        let bbox = [
            bounds.iter().map(|p| p.x).fold(f64::INFINITY, f64::min),
            bounds.iter().map(|p| p.y).fold(f64::INFINITY, f64::min),
            bounds.iter().map(|p| p.x).fold(f64::NEG_INFINITY, f64::max),
            bounds.iter().map(|p| p.y).fold(f64::NEG_INFINITY, f64::max),
        ];
    ...
    fn attribute(&mut self, entity: &AttributeEntity, layer: &LayerInfo<'a>, ctx: &Ctx<'a>, sink: &mut dyn Sink) {
        ...
        let oblique = self.style_oblique(&entity.text_style, entity.oblique_angle);
    ...
    fn style_oblique(&self, style: &str, entity_value: f64) -> f64 {
        ...
        self.doc.text_styles.get(style).map(|s| s.oblique_angle).unwrap_or(0.0)
    }
    ...
    fn mtext(&mut self, entity: &MText, layer: &LayerInfo<'a>, ctx: &Ctx<'a>, sink: &mut dyn Sink) {
        ...
        for (index, line) in lines.iter().enumerate() {
            ...
            for (dx, run) in &line.runs {
                ...
                let corners = [
                    tm.apply(Point::new(0.0, -0.25)),
                    tm.apply(Point::new(text::width_with(&run.bytes, run.font), 1.0)),
                ];
                let bbox = [
    ...
    fn raster_image(
        ...
        outline.close();
        let Some(bounds) = outline.bounds().filter(|_| outline.is_finite() && area.is_finite() && area > 1e-12) else {
