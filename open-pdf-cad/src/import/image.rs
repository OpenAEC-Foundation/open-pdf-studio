        ...
        for pixel in 0..w {
            match color_type {
                0 => {
                    ...
                    out.push(if depth < 8 { (v * 255 / max_sample) as u8 } else { v as u8 });
                }
                2 => {
                    for channel in 0..3 {
                        out.push(sample(pixel, channel)? as u8);
                    }
                }
                3 => {
...
impl ImageStore {
    ...
    pub fn get(&self, raw: &str, from: Option<&Path>, cancelled: &dyn Fn() -> bool) -> Lookup {
        ...
        let (path, unfound) = match store.paths.locate(raw, from, &IMAGE_EXTENSIONS) {
