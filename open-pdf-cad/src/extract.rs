...
impl PdfiumLibrary {
    ...
    fn annotation_candidates(&self, page: FpdfPage) -> Vec<AnnotCandidate> {
        ...
        for i in 0..count {
            ...
            let own_type = PdfiumApi::utf16_from(&buffer, (len as usize).min(buffer.len() * 2));
...
impl Walk<'_, '_> {
    ...
    fn ocg_of(&mut self, object: FpdfPageObject) -> Option<Arc<str>> {
        ...
        for i in (0..marks).rev() {
            ...
            if ok == 0 || PdfiumApi::utf16_from(&buffer, len as usize) != "OC" {
                continue;
            }
            ...
            let name = PdfiumApi::utf16_from(&buffer, len as usize);
            if name.is_empty() {
                continue;
            }
            let interned = self
