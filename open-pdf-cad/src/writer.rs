...
/// niet dubbel in het geheugen staan.
pub fn build_document(drawing: Drawing, version: CadVersion) -> Result<CadDocument, ExportError> {
...
pub fn write_drawing(drawing: Drawing, format: CadFormat, version: CadVersion, path: &Path) -> Result<u64, ExportError> {
    let doc = build_document(drawing, version)?;
    write_document(&doc, format, path)
}

pub fn write_document(doc: &CadDocument, format: CadFormat, path: &Path) -> Result<u64, ExportError> {
    // Eerst naar een tijdelijk bestand ernaast: een mislukte of afgebroken
    // export laat geen half bestand achter op de plek van een eerder resultaat.
