...
q 1 w 0 0 m 30 0 60 30 60 60 c S Q\n";
    ...
    let objects = vec![
        ...
        format!(
            ...
            if annotations { "/Annots [8 0 R 10 0 R 12 0 R 13 0 R]" } else { "" }
        ),
...
fn annotations_are_flattened_onto_their_own_layers() {
    ...
    assert_eq!(report.extract.annotations, 2, "rechthoek + tekstvak; verborgen en pop-up niet");
...
fn scan_counts_what_the_export_writes() {
    ...
    assert!(square.from_annotation && square.entities == 1);
